...
-- Icon module: 1482 Material icons + Lucide (loaded dynamically)
local IconModule = {
    Lucide   = nil,
    Material = nil, -- loaded via GetIcon()
}
...
function Luna:CreateWindow(WindowSettings)
	...
	-- Hide watermark/branding at bottom right of asset
	task.defer(function()
		for _, v in ipairs(LunaUI:GetDescendants()) do
			local name = v.Name:lower()
			if name == "watermark" or name == "branding" or name == "credit"
			or name == "footer" or name == "builtby" or name == "powered"
			or name == "made" or name == "lunatext" or name == "badge" then
	...
	-- =============================================
	-- FLUENT-STYLE API WRAPPERS
	-- Allows using Fluent syntax on Luna lib
	-- Window:AddTab(), Tab:AddSection(), Tab:AddButton(), etc.
	-- =============================================

	-- Window:AddTab({Title, Icon}) -> Tab
	function Window:AddTab(s)
		return Window:CreateTab({ Name = s.Title or s.Name or "Tab", Icon = s.Icon or "home" })
	end

	-- wrap Tab methods so they work Fluent-style
	local _origCreateTab = Window.CreateTab
	Window.CreateTab = function(self, s)
		local tab = _origCreateTab(self, s)

		-- Tab:AddSection({Name}) -> Section (also wraps Section methods)
		function tab:AddSection(s)
			local sec = self:CreateSection((type(s)=="table" and (s.Name or s.Title)) or s or "Section")

			-- Section:AddButton({Name/Title, Callback})
			function sec:AddButton(s)
				return self:CreateButton({ Name = s.Name or s.Title, Description = s.Description, Callback = s.Callback })
			end

			-- Section:AddToggle({Name/Title, Default, Callback})
			function sec:AddToggle(s)
				return self:CreateToggle({ Name = s.Name or s.Title, Description = s.Description, Default = s.Default, Callback = s.Callback })
			end

			-- Section:AddSlider({Name/Title, Min, Max, Default, Callback})
			function sec:AddSlider(s)
				return self:CreateSlider({ Name = s.Name or s.Title, Description = s.Description, Min = s.Min or 0, Max = s.Max or 100, Default = s.Default or 0, Increment = s.Increment or 1, ValueName = s.ValueName or "", Callback = s.Callback })
			end

			-- Section:AddDropdown({Name/Title, Options, Default, Callback})
			function sec:AddDropdown(s)
				return self:CreateDropdown({ Name = s.Name or s.Title, Description = s.Description, Options = s.Options or {}, Default = s.Default, Callback = s.Callback })
			end

			-- Section:AddInput({Name/Title, Placeholder, Callback})
			function sec:AddInput(s)
				return self:CreateInput({ Name = s.Name or s.Title, Description = s.Description, Placeholder = s.PlaceholderText or s.Placeholder or "Enter...", Callback = s.Callback })
			end

			-- Section:AddColorpicker({Name/Title, Default, Callback})
			function sec:AddColorpicker(s)
				return self:CreateColorPicker({ Name = s.Name or s.Title, Description = s.Description, Default = s.Default or Color3.new(1,1,1), Callback = s.Callback })
			end

			-- Section:AddKeybind({Name/Title, Default, Callback})
			function sec:AddKeybind(s)
				return self:CreateBind({ Name = s.Name or s.Title, Description = s.Description, Default = s.Default or Enum.KeyCode.Unknown, Callback = s.Callback })
			end

			-- Section:AddLabel(text)
			function sec:AddLabel(s)
				return self:CreateLabel({ Name = (type(s)=="table" and (s.Name or s.Title)) or s or "Label" })
			end

			-- Section:AddParagraph({Title, Content})
			function sec:AddParagraph(s)
				return self:CreateParagraph({ Name = s.Title or s.Name, Content = s.Content or "" })
			end

			return sec
		end

		-- Flat Tab:Add* (no section needed, like Fluent)
		function tab:AddButton(s)      return self:CreateButton({ Name = s.Name or s.Title, Description = s.Description, Callback = s.Callback }) end
		function tab:AddToggle(s)      return self:CreateToggle({ Name = s.Name or s.Title, Description = s.Description, Default = s.Default, Callback = s.Callback }) end
		function tab:AddSlider(s)      return self:CreateSlider({ Name = s.Name or s.Title, Description = s.Description, Min = s.Min or 0, Max = s.Max or 100, Default = s.Default or 0, Increment = s.Increment or 1, ValueName = s.ValueName or "", Callback = s.Callback }) end
		function tab:AddDropdown(s)    return self:CreateDropdown({ Name = s.Name or s.Title, Description = s.Description, Options = s.Options or {}, Default = s.Default, Callback = s.Callback }) end
		function tab:AddInput(s)       return self:CreateInput({ Name = s.Name or s.Title, Description = s.Description, Placeholder = s.PlaceholderText or s.Placeholder or "Enter...", Callback = s.Callback }) end
		function tab:AddColorpicker(s) return self:CreateColorPicker({ Name = s.Name or s.Title, Description = s.Description, Default = s.Default or Color3.new(1,1,1), Callback = s.Callback }) end
		function tab:AddKeybind(s)     return self:CreateBind({ Name = s.Name or s.Title, Description = s.Description, Default = s.Default or Enum.KeyCode.Unknown, Callback = s.Callback }) end
		function tab:AddLabel(s)       return self:CreateLabel({ Name = (type(s)=="table" and (s.Name or s.Title)) or s or "Label" }) end
		function tab:AddParagraph(s)   return self:CreateParagraph({ Name = s.Title or s.Name, Content = s.Content or "" }) end
